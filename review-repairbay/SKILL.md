...
description: Inspect or address actionable GitHub pull request review feedback. Use when the user wants to inspect unresolved review threads, fix selected feedback, or clear all actionable comments on a PR. Use the adapter-backed GraphQL script whenever thread-level state, resolution status, or inline review context matters.
---

# Review Repairbay

Use this skill when the user wants to work through requested changes on a GitHub pull request. The bundled script retains the GraphQL query and pagination while `github-adapter` owns authentication, transport, and PR reference resolution.
...
2. Let the bundled script resolve `repo_slug` (`OWNER/REPO`) and `pr` (number, URL, or current branch PR) through `github-adapter`; explicit URLs own their repository scope.
...
- If `github-adapter` is missing or `gh` hits auth or rate-limit issues mid-run, report the blocker and ask the user to restore the adapter or re-authenticate.

## Fallback

If `github-adapter` cannot resolve the PR cleanly, report whether repository scope, PR context, or CLI authentication is missing, then ask for the missing reference or refreshed login.
