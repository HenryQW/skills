...
6. Commit scoped fixes with Conventional Commit message(s), then push current PR branch. After confirmed push, resolve only addressed review threads with `gh api graphql` `resolveReviewThread`, then re-fetch and require each resolved. With no accepted fixes, make no commit or push. Do not reply unless explicitly requested.
