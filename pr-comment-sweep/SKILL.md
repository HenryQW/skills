...
1. Require authenticated `gh`; resolve an open PR with `gh pr view --json number,url,state,headRefName,headRefOid,comments,reviews`. Require checked-out branch equals PR head and `HEAD` equals `headRefOid`. Fetch/reset only when explicitly authorized. Inspect status, base, and full diff; stop on non-open PR, unrelated dirty changes, missing PR, or ambiguous scope.
...
6. Commit scoped fixes with Conventional Commit message(s), then push current PR branch. Before resolving, re-fetch PR metadata and require `headRefOid` equals local `HEAD`; this also permits resolution without a new push when feedback is verified addressed at current head. Resolve only addressed review threads with `gh api graphql` `resolveReviewThread`, then re-fetch and require each resolved. With no accepted fixes, make no commit or push. Do not reply unless explicitly requested.
7. Report PR URL, actioned, resolved, and skipped feedback, proposed/applied fixes, validation, commit SHA(s), push result, and blockers.
