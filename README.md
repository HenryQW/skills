...
| `code-to-pr` | Runs Greptile review loops on the current issue branch, applies actionable fixes, pushes the branch, and opens a GitHub pull request. | `npx skills install HenryQW/skills code-to-pr -a codex -y` | 2026-07-02 09:50 |
| `gh-fix-ci` | Fixes actionable GitHub PR review or CI feedback end-to-end, including commit, push, thread replies, and thread resolution. | `npx skills install HenryQW/skills gh-fix-ci -a codex -y` | 2026-07-01 19:48 |
| `gh-pr-creation` | Creates pull requests end-to-end, including scoped commits, quality checks, branch naming, PR metadata, and PR creation. | `npx skills install HenryQW/skills gh-pr-creation -a codex -y` | 2026-07-01 19:48 |
...
| `issue-to-code` | Implements a GitHub issue into a clean feature branch with a minimal intentional diff, then commits and returns only the branch name. | `npx skills install HenryQW/skills issue-to-code -a codex -y` | 2026-07-02 09:50 |
